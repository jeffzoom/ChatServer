 ...
 * @LastEditTime: 2023-10-23 10:06:03
...
bool Redis::connect() {
    ...
    // 创建一个新线程，该线程会在后台运行 observer_channel_message 函数，
    // 这个函数的任务是监听通道，处理通道上的事件，并在必要时将消息传递给业务层。
    // 这种线程可以用于异步事件处理，以不阻塞主线程的执行，从而提高程序的并发性能。
    thread t([&]() {
...
void Redis::observer_channel_message() {
    ...
    while (REDIS_OK == redisGetReply(this->_subcribe_context, (void **)&reply)) {
        ...
        if (reply != nullptr && reply->element[2] != nullptr && reply->element[2]->str != nullptr) {
            // 给业务层上报通道上发生的消息 1是通道号 2是这个通道发生的数据
